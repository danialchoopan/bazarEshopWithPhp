<?php
require_once 'php/header.php';
echo getMessageAlert()
?>
<div class="card p-2">
    <div class="row">
        <div class="m-3 ">
            <h4>تمامی محصولات و دسته بندی ها</h4>
        </div>
        <?php
        $productCategories = readAllProductCategories();
        foreach ($productCategories as $categoryProduct) {
            ?>
            <div class="m-3 ">
                <h4><?php echo $categoryProduct['name'] ?></h4>
                <hr>
            </div>
            <div class="row">
                <?php
                $products = readAllProductByCategoryId($categoryProduct['id']);
                if ($products) {
                    foreach ($products as $product) {
                        ?>
                        <a href="<?php echo APP_URL ?>product.php?product_id=<?php echo $product['id'] ?>"
                           class="text-decoration-none">
                            <div class="col-3 mb-2">
                                <div class="card h-100">
                                    <img src="<?php echo APP_URL ?>img/<?php echo $product['photo'] ?>"
                                         class="card-img-top">
                                    <div class="card-body">
                                        <h5 class="card-title" style="color: black"><?php echo $product['name'] ?></h5>
                                        <p class="card-text" style="color: black"><?php
                                            if (strlen($product['description']) > 100) {
                                                echo substr($product['description'], 100);
                                            } else {
                                                echo $product['description'];
                                            } ?> </p>
                                        <p class="m-1" style="color: #008F00">
                                            <?php echo number_format($product['price']) . "تومان" ?>
                                        </p>
                                        <div class="d-grid gap-2">
                                            <a href="?AddCartProduct_id=<?php echo $product['id'] ?>"
                                               class="btn btn-primary">افزودن به سبد خرید</a>
                                        </div>
                                    </div>
                                    <div class="card-footer">
                                        <small class="text-muted">
                                            <?php echo date('m/d/Y H:i:s', $product['created_at']) ?>
                                        </small>
                                    </div>
                                </div>
                            </div>
                        </a>

                    <?php }
                } else { ?>
                    <div class="row">
                        <div class="col">
                            <div class="alert alert-warning " role="alert">محصولی جهت نمایش وجود ندارد</div>
                        </div>
                    </div>
                <?php } ?>
            </div>
            <?php
        }
        ?>
    </div>

    <div class="m-3 ">
        <h4>دسته بندی نشده</h4>
        <hr>
        <div class="row">
            <?php
            $products = readAllProductByCategoryId(0);

            if ($products) {
                foreach ($products as $product) {
                    ?>
                    <a href="<?php echo APP_URL ?>product.php?product_id=<?php echo $product['id'] ?>"
                       class="text-decoration-none">
                        <div class="col-3 mb-2">
                            <div class="card h-100">
                                <img src="<?php echo APP_URL ?>img/<?php echo $product['photo'] ?>"
                                     class="card-img-top">
                                <div class="card-body">
                                    <h5 class="card-title" style="color: black"><?php echo $product['name'] ?></h5>
                                    <p class="card-text" style="color: black"><?php
                                        if (strlen($product['description']) > 100) {
                                            echo substr($product['description'], 100);
                                        } else {
                                            echo $product['description'];
                                        } ?> </p>
                                    <p class="m-1" style="color: #008F00">
                                        <?php echo number_format($product['price']) . "تومان" ?>
                                    </p>
                                    <div class="d-grid gap-2">
                                        <a href="?AddCartProduct_id=<?php echo $product['id'] ?>"
                                           class="btn btn-primary">افزودن به سبد خرید</a>
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <small class="text-muted">
                                        <?php echo date('m/d/Y H:i:s', $product['created_at']) ?>
                                    </small>
                                </div>
                            </div>
                        </div>
                    </a>
                <?php }
            } else { ?>
                <div class="row">
                    <div class="col">
                        <div class="alert alert-warning " role="alert">محصولی جهت نمایش وجود ندارد</div>
                    </div>
                </div>
            <?php } ?>
        </div>

    </div>
</div>
<?php
require_once 'php/footer.php'
?>
